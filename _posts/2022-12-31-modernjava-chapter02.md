...
1. `flag`가 메서드를 작성한 사람 이외에는 무엇을 의미하는지 알기 어렵다. 코드는 다른 사람도 읽기 편해야 한다고 했다. 무겁고 녹색인 사과를 원한다면 이 메서드로는 구분할 수 없다.
2. 구분점이 되는 값이 추가되면(출하지, 모양 등) 다시 메서드를 뜯어 고쳐야한다. 즉, 엔지니어링적으로 비싼 대가를 치러야 한다.
